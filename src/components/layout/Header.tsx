...
export default function Header({
  ...
  /*
    ...
    true — всегда, "xl" — только на экране от 1280px, false — никогда.

    Пунктов десять, а в строку их влезает пять: на русском и украинском
    ряд перестаёт помещаться уже на 1024px и выталкивает кнопки входа за
    край экрана — вместе с горизонтальной прокруткой на всех страницах
    сразу. Поэтому наверху то, ради чего на сайт приходят, а остальное
    живёт в подвале и в меню под аватаром.

    Шестой пункт помещается, но не везде: полоса шапки шире 1120px не
    растёт, и весь запас — это те девяносто пикселей, что появляются при
    переходе с 1024px на 1120px и дальше. Отсюда "xl": на ноутбуке ряд
    остаётся пятёркой, на большом экране показывается шестым.

    Проверять надо на польском и русском при 1280px — там слова длиннее
  ...
  */
  const supportItems = [
    ...
    // Партнёрская — шестым пунктом и только на большом экране: на
    // ноутбуке она вытолкнула бы кнопки входа за край.
    { href: `/${locale}/partner`, label: t.partner.nav, wide: "xl" as const },
            ...
            .map((item) => (
              <Link
                ...
                className={`whitespace-nowrap rounded-chip px-2.5 py-1.5 text-[13.5px] transition-colors duration-200 ${
                  item.wide === "xl" ? "hidden xl:inline-block" : ""
                } ${
                  pathname === item.href
                    ? "text-ink"
